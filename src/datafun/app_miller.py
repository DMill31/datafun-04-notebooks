...
uv run python -m datafun.app_miller
...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    LOG.info("------ Creating Histogram for Body Mass: ------------")

    # Open a fresh blank canvas before a new chart
    plt.figure()

    # Use a histplot() to visualize the distribution of a single numeric variable
    ...
    hist_plt.set_title("Distribution of Body Mass (g)")
